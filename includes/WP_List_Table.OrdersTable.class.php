...
<?php
	...
	public function column_default( $order, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'number' :
				...
				if ( $this->check_order_delete_cap() ) { $value .= ' | <a href="#" class="delete" data-id="' . esc_attr( $order->id ) . '" data-action="delete">' . __( 'Delete', 'order-tracking' ) . '</a>'; }
				$value .= '</div>';
	...
	public function get_bulk_actions() {
		...
		if ( $this->check_order_delete_cap() ) {

			$actions['delete'] =  __( 'Delete', 'order-tracking' );
		}
	...
	/**
	 * Check if the current user can delete orders assigned to them (author or above)
	 * @since 3.3.17
	 */
	public function check_order_delete_cap() {
